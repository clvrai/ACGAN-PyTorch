...
class _netG_CIFAR10_v1(nn.Module):
    def __init__(self, ngpu, nz):
        super(_netG_CIFAR10_v1, self).__init__()
        self.ngpu = ngpu
        self.nz = nz
        ...
        # Transposed Convolution 3
        self.tconv3 = nn.Sequential(
            nn.ConvTranspose2d(192, 96, 5, 2, 0, bias=False),
            nn.BatchNorm2d(96),
            nn.ReLU(True),
        )
        # Transposed Convolution 4
        self.tconv4 = nn.Sequential(
            nn.ConvTranspose2d(96, 3, 8, 2, 0, bias=False),
            nn.Tanh(),
        )
...
class _netG_CIFAR10_v2(nn.Module):
    def __init__(self, ngpu, nz):
        super(_netG_CIFAR10_v2, self).__init__()
        self.ngpu = ngpu
        self.nz = nz
...
class _netG_CIFAR10_v3(nn.Module):
    def __init__(self, ngpu, nz):
        super(_netG_CIFAR10_v3, self).__init__()
        self.ngpu = ngpu
        self.nz = nz
